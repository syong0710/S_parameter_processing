...
# renoarmalize the S-parameter
zse0_target = 46.25 # The single-ended port impedance (target imepdace)
zse0_current = abs(sedata.z0[0][0])
if zse0_target!=zse0_current:
    print('All Port Renormalize: current port impedance=' + str(zse0_current) + 'ohm; target impedance=' + str(zse0_target) + 'ohm') 
    sedata.renormalize(z_new=zse0_target)
...
# sdd11
sdd11_wf = (w_f * (sdd11**2))**0.5
sdd11_wf_integ = sum(sdd11_wf) * (1/len(sdd11_wf))
sdd11_wf_integ_db = 20*np.log10(np.abs(sdd11_wf_integ))

# sdd22
sdd22_wf = (w_f * (sdd22**2))**0.5
sdd22_wf_integ = sum(sdd22_wf) * (1/len(sdd22_wf))
sdd22_wf_integ_db = 20*np.log10(np.abs(sdd22_wf_integ))

print('integrated return loss for SDD11 = ' + str(round(sdd11_wf_integ_db,2)) + "dB")
print('integrated return loss for SDD22  = ' + str(round(sdd22_wf_integ_db,2)) + "dB")

plt.subplot(121)
plt.plot(freq/1e9, 20*np.log10(sdd11), label="SDD11", color='green')
plt.plot(freq/1e9, 20*np.log10(sdd11_wf), label="SDD11_after_WF", color='red', linestyle='dashed')
plt.title('SDD11, IRL=' + str(round(sdd11_wf_integ_db,2))+ "dB")
plt.xlabel("Frequency [GHz]")
plt.ylabel("[dB]")
plt.grid(True)
plt.legend()
...
plt.subplot(122)
plt.plot(freq/1e9, 20*np.log10(sdd22), label="SDD22", color='green')
plt.plot(freq/1e9, 20*np.log10(sdd22_wf), label="SDD22_after_WF", color='red', linestyle='dashed')
plt.title('SDD22, IRL=' + str(round(sdd22_wf_integ_db,2))+ "dB")
plt.xlabel("Frequency [GHz]")
plt.ylabel("[dB]")
plt.grid(True)
plt.legend()
